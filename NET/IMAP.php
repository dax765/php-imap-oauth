 ...
 * @link     http://pear.php.net/package/Net_IMAP
 */
class IMAP extends IMAPProtocol
{
										 ...
										 private string $_encoding = 'ISO-8859-1')
	{
		...
		if ($ret instanceof Error)
			throw new Error($ret->getMessage());
	}
	 ...
	 * @since   1.0
	 */
	public function connect($host, $port, $enableSTARTTLS = true)
	{
		...
		if ($ret === true) {
			...
			if ($this->hasCapability('STARTTLS') === true
				...
				if (($res = $this->cmdStartTLS()) instanceOf Error) {
					return $res;
				}
			}
			return $ret;
		...
		if (empty($ret)) {
			return new Error("Unexpected response on connection");
		}
		if ($ret instanceOf Error) {
			return $ret;
		}
		if (isset($ret['RESPONSE']['CODE'])) {
			if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
				return new Error($ret['RESPONSE']['CODE']
									  . ', '
	 ...
	 * @access public
	 */
	public function getSummary($msg_id = null, $uidFetch = false)
	{
		if ($msg_id != null) {
			if (is_array($msg_id)) {
				$message_set = $this->_getSearchListFromArray($msg_id);
	 ...
	 *
	 * @return mixed Either message body or Error on failure
	 * @access public
	 */
	public function getBody($msg_id, $uidFetch = false, $partId = '')
	{
		if ($partId != '') {
	 ...
	 * @access public
	 */
	public function getBodyPart($msg_id, $partId, $uidFetch = false)
	{
		if ($uidFetch) {
	 ...
	 * @access public
	 */
	public function getStructure($msg_id, $uidFetch = false)
	{
	 ...
	 * @access private
	 */
	private function _parseStructureArray($_structure, &$_mimeParts, $_partID = '')
	{
	 ...
	 * @access private
	 */
	private function _parseStructureMultipartArray($_structure,
										   &$_mimeParts,
										   $_partID,
	 ...
	 * @access private
	 */
	private function _parseStructureImageArray($_structure, &$_mimeParts, $_partID)
	{
	 ...
	 * @access private
	 */
	private function _parseStructureApplicationArray($_structure,
											 &$_mimeParts,
											 $_partID)
	{
	 ...
	 * @access private
	 */
	private function _parseStructureMessageArray($_structure, &$_mimeParts, $_partID)
	{
	 ...
	 * @access private
	 */
	private function _parseStructureTextArray($_structure, &$_mimeParts, $_partID)
	{
	...
	public function fetchMessageData($encoding, $text)
	{
		switch ($encoding) {
			case '8BIT':
	 ...
	 * @access private
	 */
	private function _parseStructureCommonFields(&$_structure)
	{
		// print "Net_IMAP::_parseStructureTextArray _partID: $_partID<br>";
		$part          = new stdClass;
	 ...
	 *
	 * @return mixed Either number of messages or Error on failure
	 *
	 * @access public
	 */
	 ...
	 * @param string $mailbox the mailbox (default is current mailbox)
	 *
	 * @return mixed Either number of messages or Error on error
	 *
	 * @access public
	 */
	public function getStatus($mailbox = '')
	{
		if ($mailbox == '') {
			$mailbox = $this->getCurrentMailbox();
		}
	 ...
	 *                          Sequence Number if set to true
	 *
	 * @return mixed Either the envelopes or Error on error
	 *
	 * @access public
	 */
	public function getEnvelope($mailbox = '', $msg_id = null, $uidFetch = false)
	{
		if ($mailbox == '') {
			$mailbox = $this->getCurrentMailbox();
		}
		...
		return new Error('Error, undefined number of messages');
	}
	 ...
	 * @access public
	 */
	public function deleteMessages($msg_id = null, $uidStore = false)
	{
		/* As said in RFC2060...
	...
	public function moveMessage($dest_mailbox, $msg_id, $isUid = false, $source_mailbox = null) {
		...
		if ($isUid)
			$ret = $this->cmdUidMove($message_set, $dest_mailbox);
		else
			$ret = $this->cmdMove($message_set, $dest_mailbox);
	 ...
	 * @since   1.0
	 */
	public function copyMessages($dest_mailbox,
						  $msg_id = null,
						  $source_mailbox = null,
						  $uidCopy = false)
	{
	 ...
	 * @since   1.0
	 */
	public function appendMessage($rfc_message, $mailbox = null, $flags_list = '')
	{
		if ($mailbox == null) {
			$mailbox = $this->getCurrentMailbox();
		}
	...
	******************************************************************/
	...
	public function getHierarchyDelimiter($mailbox = '')
	{
		...
		*/
		...
		return new Error('the IMAP Server does not support '
							  . 'HIERACHY_DELIMITER!');
	}
	 ...
	 * @since 1.0
	 */
	public function getMailboxes($reference = '',
						  ...
						  $returnAttributes = false)
	{
		...
		if (is_int($restriction_search)) {
			...
			if (is_string($restriction_search)) {
				...
				return new Error('Wrong data for 2nd parameter');
			}
		}
	 ...
	 *
	 * @return mixed boolean true/false or Error on failure
	 *
	 * @access public
	 * @since 1.0
	 */
	 ...
	 * @since 1.0
	 */
	public function createMailbox($mailbox, $options = null)
	{
	 ...
	 * @since 1.0
	 */
	public function renameMailbox($oldmailbox, $newmailbox, $options = null)
	{
	...
	******************************************************************/
	 ...
	 * @since 1.0
	 */
	public function listsubscribedMailboxes($reference = '',
									 $restriction_search = 0,
									 $returnAttributes = false)
	{
		if (is_bool($restriction_search)) {
	...
	******************************************************************/
	 ...
	 * @since 1.0
	 */
	public function getFlags($msg_id = null, $includeUid = false)
	{
	 ...
	 * @since 1.1
	 */
	public function setFlags($msg_id, $flags, $mod = 'set', $uidStore = false)
	{
		// you can also provide an array of numbers to those emails
		if ($msg_id == 'all') {
			$message_set = '1:*';
		...
		default:
			// Wrong Input
			return new Error('wrong input for $mod');
			break;
		}
	 ...
	 * @since 1.1
	 */
	public function addFlags($msg_id, $flags)
	{
		return $this->setFlags($msg_id, $flags, 'add');
	}
	 ...
	 * @since 1.1
	 */
	public function addSeen($msg_id)
	{
		return $this->setFlags($msg_id, '\Seen', 'add');
	}
	 ...
	 * @since 1.1
	 */
	public function addAnswered($msg_id)
	{
		return $this->setFlags($msg_id, '\Answered', 'add');
	}
	 ...
	 * @since 1.1
	 */
	public function addDeleted($msg_id)
	{
		return $this->setFlags($msg_id, '\Deleted', 'add');
	}
	 ...
	 * @since 1.1
	 */
	public function addFlagged($msg_id)
	{
		return $this->setFlags($msg_id, '\Flagged', 'add');
	}
	 ...
	 * @since 1.1
	 */
	public function addDraft($msg_id)
	{
		return $this->setFlags($msg_id, '\Draft', 'add');
	}
	 ...
	 * @since 1.1
	 */
	public function removeFlags($msg_id, $flags)
	{
		return $this->setFlags($msg_id, $flags, 'remove');
	}
	 ...
	 * @since 1.1
	 */
	public function removeSeen($msg_id)
	{
		return $this->setFlags($msg_id, '\Seen', 'remove');
	}
	 ...
	 * @since 1.1
	 */
	public function removeAnswered($msg_id)
	{
		return $this->setFlags($msg_id, '\Answered', 'remove');
	}
	 ...
	 * @since 1.1
	 */
	public function removeDeleted($msg_id)
	{
		return $this->setFlags($msg_id, '\Deleted', 'remove');
	}
	 ...
	 * @since 1.1
	 */
	public function removeFlagged($msg_id)
	{
		return $this->setFlags($msg_id, '\Flagged', 'remove');
	}
	 ...
	 * @since 1.1
	 */
	public function removeDraft($msg_id)
	{
		return $this->setFlags($msg_id, '\Draft', 'remove');
	}
	...
	{
		if ($resp = $this->getFlags($message_nro) instanceOf Error) {
			return $resp;
		}
		if (isset($resp[0])) {
			if (is_array($resp[0])) {
	...
	******************************************************************/
	...
	/**
	 * Search public function. Sends the SEARCH command
	 *
	 ...
	 * @since 1.1
	 */
	public function sort($sort_list,
				  $charset = 'US-ASCII',
				  $search_list = '',
				  $uidSort = false)
	{
	...
	******************************************************************/
	 ...
	 *                             current mailbox)
	 *
	 * @return array contaning the quota info on success or
	 *                      Error on failure
	 *
	 * @access public
	 * @since 1.0
	 */
	public function getStorageQuotaRoot($mailbox_name = null)
	{
		...
		if ($ret instanceOf Error) {
			return new Error($ret->getMessage());
		}
	 ...
	 *                             current mailbox)
	 *
	 * @return array contaning the quota info on success or Error
	 *                     on failure
	 *
	 * @access public
	 * @since 1.0
	...
	******************************************************************/
	 ...
	 *                             current mailbox)
	 *
	 * @return mixed string on success or false or Error on failure
	 *
	 * @access public
	 * @since 1.0
	 */
	...
	public function getMyRights($mailbox_name = null)
	{
		...
		return new Error('Bogus response from server!');
	}
	...
	******************************************************************/
	 ...
	 * @access private
	 */
	private function _getSearchListFromArray($arr)
	{
		$txt = implode(',', $arr);
		return $txt;
	}
	...
	/*****************************************************
		Net_POP3 Compatibility public functions:
	...
	*********************************************************/
	 ...
	 * Net_POP3 Compatibility public function
	 *
	 * @param string $mailbox Mailbox (default is current mailbox)
	 *
	 * @return mixed Either number of messages or Error on failure getNumberOfMessages($mailbox)
	 *
	 * @access public
	 */
