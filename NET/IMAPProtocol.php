...
class IMAPProtocol
{
	 ...
	 * @var array
	 */
	public $supportedAuthMethods = array('DIGEST-MD5', 'CRAM-MD5', 'LOGIN', 'XOAUTH2');
	 ...
	 * @access private
	 */
	private $_serverAuthMethods = null;
	 ...
	 * @access private
	 */
	private $_streamContextOptions = null;
	 ...
	 * @access private
	 */
	private $_serverSupportedCapabilities = null;
	 ...
	 *
	 * @return mixed Returns a Error with an error message on any
	...
	{
		if ($this->_connected) {
			return new Error('already connected, logout first!');
		}
		...
		if ($port == 993) {
			if (!$this->_socket->enableCrypto(true, STREAM_CRYPTO_METHOD_TLS_CLIENT)) {
				return new Error('Failed to set crypto');
			}
		}

		if ($this->_getRawResponse() instanceOf Error) {
			return new Error('unable to open socket');
		}
	 ...
	 * @since 1.1
	 */
	public function setPrintErrors($printErrors = true)
	{
	 ...
	 *
	 * @return mixed True on success or a Error object on failure.
	 *
	 * @access private
	 * @since 1.0
	 */
	...
	{
		if ($this->_socket->eof()) {
			return new Error('Failed to write to socket: (connection lost!)');
		}
		$error = $this->_socket->write($data);
		if ($error instanceOf Error) {
			return new Error('Failed to write to socket: '
								  . $error->getMessage());
		}
	 ...
	 *
	 * @return mixed a line of response on success or a Error object on failure.
	 *
	 * @access private
	 * @since 1.0
	 */
	private function _recvLn()
	{
		$this->lastline = $this->_socket->gets(8192);
		if ($this->lastline instanceOf Error) {
			return new Error('Failed to write to socket: '
		...
		if ($this->lastline == '') {
			return new Error('Failed to receive from the  socket: ');
		}
	 ...
	 * @since 1.0
	 */
	private function _putCMD($commandId , $command, $args = '')
	{
		if (!empty($args)) {
	 ...
	 * @access private
	 */
	private function _getRawResponse($commandId = '*')
	{
		$arguments = '';
		while (!$this->_recvLn() instanceOf Error) {
	...
	public function _authXOAUTH2($uid, $pwd, $cmdid) {
		$authStrToken = base64_encode("user={$uid}\1auth=Bearer {$pwd}\1\1");
		$authStr = "{$cmdid} AUTHENTICATE XOAUTH2 {$authStrToken}";
		$error = $this->_send($authStr . "\r\n");
		$args = $this->_recvLn();
		$plus = $space = $challenge = '';
	 ...
	 * @since  1.0
	 */
	private function _authDigestMD5($uid, $pwd, $cmdid)
	{
	 ...
	 * @since 1.0
	 */
	private function _authCramMD5($uid, $pwd, $cmdid)
	{
		$error = $this->_putCMD($cmdid,
									  'AUTHENTICATE',
									  'CRAM-MD5');
	 ...
	 * @since 1.0
	 */
	private function _authLOGIN($uid, $pwd, $cmdid)
	{
		$error = $this->_putCMD($cmdid,
										'AUTHENTICATE',
										'LOGIN');
	 ...
	 *               supported authentication method or a Error object
	 *               if a failure condition is encountered.
	 * @access private
	 * @since 1.0
	 */
	private function _getBestAuthMethod($userMethod = null)
	{
		$this->cmdCapability();
		...
		if (($methods != null) && ($this->_serverAuthMethods != null)) {
			...
			return new Error($method . ' NOT supported authentication'
								  . ' method! This IMAP server supports these'
								  . ' methods: ' . $serverMethods . ', but I'
								  . ' support ' . $myMethods);
		} else {
			return new Error('This IMAP server don\'t support any Auth'
								  . ' methods');
		}
	}
	...
	public function cmdLogout()
	{
		...
		if ($this->_socket->disconnect() instanceOf Error) {
			return new Error('socket disconnect failed');
		}
	 ...
	 * @since 1.0
	 */
	public function cmdCreate($mailbox, $options = null)
	{
		$args         = '';
		$mailbox_name = $this->_createQuotedString($mailbox);
	 ...
	 * @since  1.0
	 */
	public function cmdRename($mailbox, $new_mailbox, $options = null)
	{
		$mailbox_name     = $this->_createQuotedString($mailbox);
	...
	public function cmdStatus($mailbox, $request)
	{
		...
		if (!$ret instanceOf Error && isset($ret['PARSED'])) {
	...
	public function cmdExpunge()
	{
		...
		if ($ret instanceOf Error) {
			return new Error('could not Expunge!');
		}
	 ...
	 * @since 1.1
	 */
	public function cmdSort($sort_cmd)
	{
		/*
		if ($_charset != '' )
			$_charset = "[$_charset] ";
	...
	public function cmdMove($msgID, $destMailbox) {
		$mailbox_name = $this->_createQuotedString($destMailbox);
		return $this->_genericCommand('MOVE',
												sprintf("%s %s",
														  $msgID,
														  $mailbox_name));
	}

	public function cmdUidMove($msgID, $destMailbox) {
		$mailbox_name = $this->_createQuotedString($destMailbox);
		return $this->_genericCommand('UID MOVE',
												sprintf("%s %s",
														  $msgID,
														  $mailbox_name));
	}
	 ...
	 * @since 1.1
	 */
	public function cmdUidSort($sort_cmd)
	{
	...
	/********************************************************************
	***
	**             HERE ENDS the RFC2060 IMAPS public functionS
	...
	*******************************************************************/
	...
	public function cmdGetQuota($mailbox_name)
	{
		...
		if (!$this->hasQuotaSupport()) {
			return new Error('This IMAP server doen\'t support QUOTA\'s!');
		}
	...
	public function cmdGetQuotaRoot($mailbox_name)
	{
		//Check if the IMAP server has QUOTA support
		if (!$this->hasQuotaSupport()) {
			return new Error('This IMAP server doesn\'t support QUOTA\'s!');
		}
	 ...
	 * @since 1.0
	 */
	public function cmdSetQuota($mailbox_name,
						 $storageQuota = null,
						 $messagesQuota = null)
	{
		...
		if (($messagesQuota == null) && ($storageQuota == null)) {
			return new Error('$storageQuota and $messagesQuota parameters '
	 ...
	 * @since 1.0
	 */
	public function cmdSetQuotaRoot($mailbox_name,
							 $storageQuota = null,
							 $messagesQuota = null)
	{
		//Check if the IMAP server has QUOTA support
	...
	********************************************************************/
	...
	public function cmdSetACL($mailbox_name, $user, $acl)
	{
		...
		if (!$this->hasAclSupport()) {
			return new Error('This IMAP server does not support ACL\'s!');
		}
	...
	********************************************************************/
	 ...
	 * @since 1.0
	 */
	public function cmdSetAnnotation($mailboxName, $entry, $values)
	{
		// Check if the IMAP server has ANNOTATEMORE support
		if (!$this->hasAnnotateMoreSupport()) {
			return new Error('This IMAP server does not support the '
								  . 'ANNOTATEMORE extension!');
		}

		if (!is_array($values)) {
			return new Error('Invalid $values argument passed to '
								  . 'cmdSetAnnotation');
		}
	 ...
	 * @since 1.0
	 */
	public function cmdDeleteAnnotation($mailboxName, $entry, $values)
	{
		// Check if the IMAP server has ANNOTATEMORE support
		if (!$this->hasAnnotateMoreSupport()) {
			return new Error('This IMAP server does not support the '
	 ...
	 * @since 1.0
	 */
	public function cmdGetAnnotation($mailboxName, $entries, $values)
	{
		// Check if the IMAP server has ANNOTATEMORE support
		if (!$this->hasAnnotateMoreSupport()) {
			return new Error('This IMAP server does not support the '
	...
	************************************************************************/
	...
	/********************************************************************
	***
	***             HERE ENDS THE EXTENSIONS public functionS
	...
	********************************************************************/
	 ...
	 * @since 1.0
	 */
	public function _createQuotedString($str)
	{
		$search  = array('\\', '"');
		$replace = array('\\\\', '\\"');
	 ...
	 * @since 1.0
	 */
	private function _parseOneStringResponse(&$str, $line, $file)
	{
		$this->_parseSpace($str, $line, $file);
		$uid = false;
		$this->_getNextToken($str, $uid);
		return $uid;
	}
	 ...
	 * @since 1.0
	 */
	private function _parseFLAGSresponse(&$str)
	{
	 ...
	 * @since 1.0
	 */
	private function _parseBodyResponse(&$str, $command)
	{
		$this->_parseSpace($str, __LINE__, __FILE__);
		while ($str[0] != ')' && $str != '') {
			$params_arr[] = $this->_arrayfyContent($str);
	 ...
	 * @since 1.0
	 */
	private function _arrayfyContent(&$str)
	{
		$params_arr = [];
		$params = false;
	 ...
	 * @since 1.0
	*/
	private function _parseContentresponse(&$str, $command)
	{
		$content = '';
		$this->_parseSpace($str, __LINE__, __FILE__);
	 ...
	 * @since 1.0
	 */
	private function _parseENVELOPEresponse(&$str)
	{
		//$content = '';
		$this->_parseSpace($str, __LINE__, __FILE__);
		$parenthesis = false;
	 ...
	 * @since 1.0
	 */
	private function _getAddressList(&$str)
	{
		$params_arr = $this->_arrayfyContent($str);
		if (!isset($params_arr)) {
			return $params_arr;
	 ...
	 * @since 1.0
	*/
	private function _getClosingBracesPos($str_line,
								  $startDelim = '(',
								  $stopDelim = ')')
	{
	 ...
	 * @since 1.1
	 */
	private function _advanceOverStr($str,
							 &$pos,
							 $len,
							 $startDelim ='(',
							 $stopDelim = ')')
	 ...
	 * @since  1.0
	 */
	private function _getToEOL(&$str, $including = true)
	{
	 ...
	 * @since 1.0
	 */
	private function _getNextToken(&$str,
											&$content,
	 ...
	 * @since 1.0
	 */
	private function _protError($str , $line , $file, $printError = true)
	{
	 ...
	 * @since 1.0
	 */
	private function _getEXTarray(&$str, $startDelim = '(', $stopDelim = ')')
	{
	 ...
	 * @since 1.0
	 */
	private function _retrParsedResponse(&$str, $token, $previousToken = null)
	{
		//echo "\n\nTOKEN:$token\r\n";
		$token = strtoupper($token);
		...
		case 'LIST':
			...
			$this->_parseSpace($str, __LINE__, __FILE__);
			$hierarchydelim = $mailbox_name = false;
		...
		case 'ACL':
			...
			*/
			...
			while (substr($str, 0, 2) != "\r\n") {
				$acl_user = $acl_rights = false;
				$this->_parseSpace($str, __LINE__, __FILE__);
				$this->_getNextToken($str, $acl_user);
				$this->_parseSpace($str, __LINE__, __FILE__);
				$this->_getNextToken($str, $acl_rights);
				$arr[] = ['USER' => $acl_user, 'RIGHTS' => $acl_rights];
			}

			$result_array = ['MAILBOX' => $this->utf7Decode($mailbox),
								  'USERS'   => $arr];
			return $result_array;
			break;
	 ...
	 * @access private
	 */
	private function _parseSpace(&$str, $line, $file, $printError = true)
	{
		/*
		This code repeats a lot in this class
		so i make it a public function to make all the code shorter
		*/
		$space = false;
	 ...
	 * @access private
	 */
	private function _parseString(&$str, $char, $line, $file)
	{
		/*
		This code repeats a lot in this class
		so i make it a public function to make all the code shorter
	 ...
	 * @access private
	 */
	private function _genericImapResponseParser(&$str, $cmdid = null)
	{
		$result_array = [];
	 ...
	 * @access private
	 */
	private function _genericCommand($command, $params = '')
	{
		if (!$this->_connected) {
			return new Error("not connected! (CMD:$command)");
		}
		$cmdid = $this->_getCmdId();
	 ...
	 * @access public
	 */
	public function utf7Encode($str)
	{
	 ...
	 * @access public
	 */
	public function utf7Decode($str)
	{
		if ($this->_useUTF_7 == false) {
			return $str;
		}
	 ...
	 * @since 1.1
	 */
	private function _getCreateParams($options)
	{
		$args = '';
	 ...
	 * @access private
	 */
	private function _getLineLength($string)
	{
	 ...
	 * @access private
	 */
	private function _getSubstr($string, $start, $length = false)
	{
		if (extension_loaded('mbstring')) {
			if ($length !== false) {
